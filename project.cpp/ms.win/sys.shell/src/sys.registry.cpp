/*
	...
	The is system registry storage wrapper interface interface implementation file;
*/
...
rect_t  CRegKey_Ex::CValue::GetRect (_pc_sz _p_key_path) {
	_p_key_path;
...
err_code CRegKey_Ex::CValue::Set (_pc_sz _p_key_path, const rect_t& _rect) {
	_p_key_path; _rect;
	m_the_key.m_error <<__METHOD__<<__s_ok;
...
bool CRegKey_Ex::Is_exist (_pc_sz _p_key_path) const {
	...
	b_result = 0 == CRegKey().Open(CRoot().Key(), _p_key_path); // cannot use m_key field due to this is the 'const' method;
	return b_result;
}
...
err_code CRegKey_Ex::Open (_pc_sz _p_key_path) {
	...
	if (this->Is_exist(_p_key_path) == false) {
		if (0 != CRegKey().Create(CRoot().Key(), _p_key_path))
