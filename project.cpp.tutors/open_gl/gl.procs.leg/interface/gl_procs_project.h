...
namespace open_gl { namespace procs { namespace projection { namespace ver_1_1 {
	...
	*/
	...
	public:
		...
		const
		f_seq_4x4& operator <<(const s_pers_args&); // generates the perspective matrix by setting input args and returns the matrix being generated;
		const
		CMode& Mode (void) const; // returns the reference to matrix mode object; (ro)
		CMode& Mode (void);       // returns the reference to matrix mode object; (rw)
