...
err_code CTstMatMode::Get (void) {
	...
	if (__failed((*this)().Get())) { _out() += (*this)().Error(); }
		...
		_out() += TString().Format(_T("[impt] current matrix stack mode: '%s';"), CMode::To_str(n_current));
	}
...
err_code CTstMatMode::Set (const e_stk_modes _e_mode) {
	...
	if (__failed((*this)().Set(_e_mode))) { _out() += (*this)().Error(); }
		...
		_out() += TString().Format(_T("[impt] result matrix stack mode: '%s' is set;"), CMode::To_str(_e_mode));
	}
...
const
CMode& CTstMatMode::operator ()(void) const { return this->m_mode; }
CMode& CTstMatMode::operator ()(void)       { return this->m_mode; }
...
err_code CTstMatrix::Get (const e_mat_type _type) {
	...
	if (this->m_verbose)
	_out() += TString().Format(_T("[warn] cls::[%s::%s].%s():"), (_pc_sz)__SP_NAME__, (_pc_sz)__CLASS__, (_pc_sz)__METHOD__);
	_out() += TString().Format(_T("Querying matrix of type '%s':"), (_pc_sz) CType::To_str(_type));
