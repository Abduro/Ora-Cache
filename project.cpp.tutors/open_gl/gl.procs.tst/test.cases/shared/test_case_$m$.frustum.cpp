...
CTstFrustum::CTstFrustum (const bool _b_verbose) : m_verbose(_b_verbose) { _out().Clear(); }

err_code  CTstFrustum::Set (const f_rect& _clip, const f_planes& _planes) {
	_clip; _planes;
	if (this->m_verbose)
	_out() += TString().Format(_T("[warn] cls::[%s::%s].%s():"), (_pc_sz)__SP_NAME__, (_pc_sz)__CLASS__, (_pc_sz)__METHOD__);
	_out() += TString().Format(_T("Input args: clip rect = %s; planes = {%s};"), _clip.To_str(), _planes.To_str());
...
err_code  CTstFrustum::Set (const t_rect& _clip, const f_planes& _planes) {
	_clip; _planes;
...
const
CFrustum& CTstFrustum::operator ()(void) const { return this->m_frustum; }
CFrustum& CTstFrustum::operator ()(void)       { return this->m_frustum; }
