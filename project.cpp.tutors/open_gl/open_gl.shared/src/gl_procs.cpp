...
enum class e_prog_fun_ndx : uint32_t {
	e_attach  = 0x0, e_att_loc = 0x1, e_create = 0x2, e_delete = 0x3, e_info_log = 0x4, e_link = 0x5, e_params = 0x6, e_use = 0x7, e_valid = 0x8,
};
...
err_code CProg::Attach (uint32_t _prog_id, uint32_t _shader_id) {
	...
	*/
	...
	pfn_Attach p_fun = reinterpret_cast<pfn_Attach>(CBase::Get(prog_fun_names[(uint32_t)e_prog_fun_ndx::e_attach]));
	if (nullptr == p_fun)
		return CBase::Error();
...
int32_t  CProg::AttLocate (uint32_t _prog_id, char* _p_name) {
	...
	*/
	...
	pfn_AttLocate  p_fun = reinterpret_cast<pfn_AttLocate>(CBase::Get(prog_fun_names[(uint32_t)e_prog_fun_ndx::e_att_loc]));
	if (nullptr == p_fun)
		return n_result;
...
err_code CProg::Link (uint32_t _prog_id){
	...
	*/
	...
	pfn_Link p_fun = reinterpret_cast<pfn_Link>(CBase::Get(prog_fun_names[(uint32_t)e_prog_fun_ndx::e_link]));
	if (nullptr == p_fun)
		return CBase::Error();
