...
enum e_shader_fun_ndx : uint32_t {
...
uint32_t  CShader::Create  (uint32_t _u_type) {
	...
	*/
	...
	pfn_Create p_fun = reinterpret_cast<pfn_Create>(CBase::Get(shader_fun_names[e_shader_fun_ndx::e_create]));
...
err_code  CShader::Delete  (uint32_t _shader_id) {
	...
	pfn_Delete p_fun = reinterpret_cast<pfn_Delete>(CBase::Get(shader_fun_names[e_shader_fun_ndx::e_delete]));
	if (nullptr == p_fun)
		return CBase::Error();
...
err_code  CShader::InfoLog (uint32_t _shader_id, int32_t _n_max_length, int32_t* _p_log_len, char* _p_log) {
	...
	*/
	...
	pfn_InfoLog p_fun = reinterpret_cast<pfn_InfoLog>(CBase::Get(shader_fun_names[e_shader_fun_ndx::e_infolog]));
	if (nullptr == p_fun)
		return CBase::Error();
...
bool   CShader::Is_valid  (uint32_t _shader_id) {
	...
	pfn_IsValid p_fun = reinterpret_cast<pfn_IsValid>(CBase::Get(shader_fun_names[e_shader_fun_ndx::e_is_shader]));
	if (nullptr == p_fun) {
		return false;
	}
...
err_code  CShader::Params  (uint32_t _shader_id, uint32_t _param_type, int32_t* _p_params) {
	...
	/* Possible errors:
		...
		GL_INVALID_OPERATION : _shader_id does not refer to a shader object ;
	*/
	pfn_Params p_fun = reinterpret_cast<pfn_Params>(CBase::Get(shader_fun_names[e_shader_fun_ndx::e_params]));
	if (nullptr == p_fun)
		return CBase::Error();
...
err_code  CShader::Source  (uint32_t _shader_id, int32_t _n_count, const char** const _p_string, const int32_t* _p_length) {
	...
	*/
	...
	pfn_Source p_fun = reinterpret_cast<pfn_Source>(CBase::Get(shader_fun_names[e_shader_fun_ndx::e_source]));
	if (nullptr == p_fun)
		return CBase::Error();
