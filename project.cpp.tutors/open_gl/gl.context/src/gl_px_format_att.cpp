/*
	Created by Tech_dog (ebontrop@gmail.com) on 23-May-2026 at 13:52:10.585, UTC+4, Batumi, Saturday;
	This is OpenGL ARB extension format attributes' wrapper interface implementation file;
*/
#include "gl_px_format_att.h"

using namespace open_gl::arb::format;
using namespace open_gl::arb::format::atts;
...
CAtt_set_ctx:: CAtt_set_ctx (const uint32_t _u_major/* = 1*/, const uint32_t _u_minor/* = 1*/) : TBase() {
	...
	if (__is_okay(n_result)) n_result = TBase::Append(CAtt(atts::CContext::CVersion::e_major, _u_major, (_pc_sz) atts::CContext::CVersion::To_str(atts::CContext::CVersion::e_major)));
	if (__is_okay(n_result)) n_result = TBase::Append(CAtt(atts::CContext::CVersion::e_minor, _u_minor, (_pc_sz) atts::CContext::CVersion::To_str(atts::CContext::CVersion::e_minor)));
	if (__is_okay(n_result)) n_result = TBase::Append(
		CAtt(atts::CContext::CProfile::e_mask, atts::CContext::CProfile::e_core, (_pc_sz) atts::CContext::CProfile::To_str(atts::CContext::CProfile::e_mask)));
	if (__is_okay(n_result)) n_result = TBase::Append(CAtt(atts::CContext::e_flags, 0, (_pc_sz) atts::CContext::To_str(atts::CContext::e_flags)));
}

CAtt_set_pixels:: CAtt_set_pixels (const s_bits_ex& _args) : TBase() {
	_args; // is not used yeat;
	err_code n_result = __s_ok;

	if (__is_okay(n_result)) n_result = TBase::Append(CAtt(atts::CDraw::e_window   , _gl_true, (_pc_sz) atts::CDraw::To_str(atts::CDraw::e_window)));
	if (__is_okay(n_result)) n_result = TBase::Append(CAtt(atts::CSupport::e_opengl, _gl_true, (_pc_sz) atts::CSupport::To_str(atts::CSupport::e_opengl)));
	if (__is_okay(n_result)) n_result = TBase::Append(CAtt(atts::CBuffer::e_double , _gl_true, (_pc_sz) atts::CBuffer::To_str(atts::CBuffer::e_double)));
	if (__is_okay(n_result)) n_result = TBase::Append(CAtt(atts::CType::e_pixel    , atts::CType::e_rgba , (_pc_sz) atts::CType::To_str(atts::CType::e_pixel)));
	// sets color related attributes;
	if (__is_okay(n_result)) n_result = TBase::Append(CAtt(atts::CColor::e_alpha   ,  8, (_pc_sz) atts::CColor::To_str(atts::CColor::e_alpha)));
	if (__is_okay(n_result)) n_result = TBase::Append(CAtt(atts::CColor::e_color   , 32, (_pc_sz) atts::CColor::To_str(atts::CColor::e_color)));
	if (__is_okay(n_result)) n_result = TBase::Append(CAtt(atts::CColor::e_depth   , 24, (_pc_sz) atts::CColor::To_str(atts::CColor::e_depth)));
	// sets MSAA related attributes;
	if (__is_okay(n_result)) n_result = TBase::Append(CAtt(atts::CSample::e_multi  ,  1, (_pc_sz) atts::CSample::To_str(atts::CSample::e_multi))); // must be 1 for MSAA;
	if (__is_okay(n_result)) n_result = TBase::Append(CAtt(atts::CSample::e_count  ,  8, (_pc_sz) atts::CSample::To_str(atts::CSample::e_count))); // must be power of 2: 2,4,8,16;
}
