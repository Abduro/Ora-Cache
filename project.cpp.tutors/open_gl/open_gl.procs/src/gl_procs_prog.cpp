...
err_code CInterface::Get (const uint32_t _prog_id, const e_interface _e_iface, const e_property _e_prop, uint32_t& _u_value) {
	...
	*/
	...
	switch ( u_err_code ) { // not applicable pairs of the property value with the interface one are not taking into account yet;
...
err_code CProg::InfoLog (const uint32_t _prog_id, const int32_t _buf_size, uint32_t* _log_len, char* _p_log){
...
err_code CProg::Params  (const uint32_t _prog_id, const uint32_t _param_id, uint32_t* _p_params){
	...
	*/
	...
	p_fun(_prog_id, _param_id, reinterpret_cast<int32_t*>(_p_params)); // there is no reason for assuming the pointer to array or value of 'signed' data types;
	const
	uint32_t u_err_code = CErr_ex().Get_code();
	switch ( u_err_code ) {
