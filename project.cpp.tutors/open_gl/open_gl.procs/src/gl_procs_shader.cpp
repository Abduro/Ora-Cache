...
uint32_t  CShader::Create (const uint32_t _u_type) {
	...
	/* Possible errors:
		...
		GL_INVALID_ENUM : '_u_type' is not an accepted value ;
	*/
...
err_code  CShader::InfoLog (uint32_t _shader_id, uint32_t _u_max_len, int32_t* _p_log_len, char* _p_log) {
	_shader_id; _u_max_len; _p_log_len; _p_log;
	/* Possible errors:
		GL_INVALID_VALUE     : '_shader_id' is not a value generated by OpenGL ;
		GL_INVALID_OPERATION : '_shader'_id does not refer to a shader object ;
		GL_INVALID_VALUE     : '_u_max_len' is less than 0 ; the data type of this argument intentionally changed to 'unsigned' in order to avoid this type of error;
	*/
	CBase::m_error << __METHOD__ << __s_ok;
	...
	p_fun(_shader_id, static_cast<int32_t>(_u_max_len), _p_log_len, _p_log);
	const
	uint32_t u_err_code = CErr_ex().Get_code();
	switch ( u_err_code ){
...
bool  CShader::Is_valid (uint32_t _shader_id) {
	...
	if (1 > _shader_id) {
		CBase::m_error <<__e_inv_arg = _T("#__e_inv_val: '_shader_id' has default value 0 (zero)"); return false;
	}
