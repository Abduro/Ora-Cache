...
bool CBuffer::Is_Buffer (const uint32_t _u_buf_id) {
	...
	if (0 == _u_buf_id) {
		CBase::m_error <<__e_inv_arg = TString().Format(_T("#__e_inv_arg: '_u_buf_id' (%u) uses reserved identifier"), _u_buf_id);
		return false;
	}
	...
	/* the excerpt from https://registry.khronos.org/OpenGL-Refpages/gl4/html/glIsBuffer.xhtml :
