...
err_code shader::CWnd::PostCreate (void) {
	...
	if (__failed(this->Renderer().Scene().Progs().Get(e_object::e_tria).Shaders().Fragment().Src().Cfg().ResId(IDS_TUTOR_2_SHADER_FRAG_0, e_res_types::e_string)))
	    __trace_err_2(_T("%s\n"), (_pc_sz) this->Renderer().Scene().Progs().Get(e_object::e_tria).Shaders().Fragment().Src().Cfg().Error().Print(TError::e_print::e_req));
	if (__failed(this->Renderer().Scene().Progs().Get(e_object::e_tria).Shaders().Vertex().Src().Cfg().ResId(IDS_TUTOR_2_SHADER_VERT_0, e_res_types::e_string)))
	    __trace_err_2(_T("%s\n"), (_pc_sz) this->Renderer().Scene().Progs().Get(e_object::e_tria).Shaders().Vertex().Src().Cfg().Error().Print(TError::e_print::e_req));
#else
	// sets the option of the shader source configuration to use external shader source files;
	CProg_enum& progs = this->Renderer().Scene().Progs();
	if (__failed(progs.Get(e_object::e_tria).Shaders().Fragment().Src().Cfg().Path($Type::e_fragment)))
	    __trace_err_2(_T("%s\n"), (_pc_sz) progs.Get(e_object::e_tria).Shaders().Fragment().Src().Cfg().Error().Print(TError::e_print::e_req));
	if (__failed(prog.Shaders().Vertex().Src().Cfg().Path($Type::e_vertex)))
	    __trace_err_2(_T("%s\n"), (_pc_sz) progs.Get(e_object::e_tria).Shaders().Vertex().Src().Cfg().Error().Print(TError::e_print::e_req));
#endif
