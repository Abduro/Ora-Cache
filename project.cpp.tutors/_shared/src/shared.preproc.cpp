/*
	Created by Tech_dog (ebontrop@gmail.com) on 10-Apr-2024 09:24:03.506, UTC+7, Novosibirsk, Wednesday;
...
namespace shared { namespace common { namespace _impl {
	...
	public:
		bool PutApart (_pc_sz const _p_spec) {
			...
			TParts  elements_ = this->Split(_p_spec, s_colons, false);
	...
	public:
		...
		const TParts&  Split(_pc_sz _p_src, _pc_sz _p_sep, const bool _b_preserve_sep = false) const {
			_p_src; _p_sep; _b_preserve_sep;

			if (this->m_parts.empty() == false)
				this->m_parts.clear();

			if (NULL == _p_src || 0 == ::lstrlen(_p_sep))
				return this->m_parts;

			CString cs_src(_p_src);
			INT n_pos = 0;
			CString cs_item = cs_src.Tokenize(_p_sep, n_pos); if (cs_item.IsEmpty() == false) cs_item.Trim();

			while (cs_item.IsEmpty() == false) {
				try {
					if (_b_preserve_sep) {
						CString cs_preserved = cs_item; cs_preserved += _p_sep;
						this->m_parts.push_back(cs_preserved);
					}
					else
						this->m_parts.push_back(cs_item);
				}
				catch (::std::bad_alloc&) {
					break;
				}
				cs_item = cs_src.Tokenize(_p_sep, n_pos); if (cs_item.IsEmpty() == false) cs_item.Trim();
			}
			return this->m_parts;
		}  
...
CString CPreprocessor::Method (_pc_sz const _p_spec) { return CSpecParser(_p_spec).Method(); }
