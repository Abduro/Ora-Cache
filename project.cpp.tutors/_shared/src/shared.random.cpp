...
#pragma region cls::CRandom{}

CRandom:: CRandom (void) { RandomValue_Initialize(); }
CRandom::~CRandom (void) {}

long    CRandom::SignedLong (const long _min, const long _max) const {
	_min; _max;
...
CString CRandom::UniqueSeq (const dword _length, const bool bCaseSensitive) const {
...
CString CRandom::UniqueSeq_Ex (const dword _length, const bool bLowCase) const {
	_length; bLowCase;
...
_uint   CRandom::Unsigned (const _uint _min, const _uint _max) const
{
	if (_max < _min || _max == _min)
		return 0;
	::srand((unsigned)::std::time(0));
...
CString CRandom::GuidAsText(const bool _b_simple , const bool bCaseSensitive) {
...
CString CRandom::GuidAsText(const GUID& _guid_ref, const bool _b_simple , const bool bCaseSensitive) {
...
#pragma endregion
#pragma region cls::CRandom_2{}

CRandom_2::CRandom_2(void) : TBase() {}

double CRandom_2::Epsilon (void) {
...
float  CRandom_2::Float (const float _f_min, const float _f_max) {
	_f_min; _f_max;
#if (1)
	// std::random_device is used for a non-deterministic seed (if available);
	::std::random_device  rnd_dev;
	::std::mt19937 engine(rnd_dev()); // seeds the Mersenne Twister engine;
#else
	// fallback to time seed if random_device isn't implemented well (e.g., MinGW on Windows);
	::std::mt19937 engine(static_cast<unsigned int>(::std::time(nullptr)));
#endif
	// defines the uniform real distribution between min and max (half-open interval [min, max));
    ::std::uniform_real_distribution<float> distro(_f_min, _f_max);
