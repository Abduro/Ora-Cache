...
_pc_sz CErr_Details::args (_pc_sz _lp_sz_pat, const va_list& _err_args) {
	...
	try {
		...
		do
		{
			...
			// https://learn.microsoft.com/en-us/windows/win32/api/strsafe/nf-strsafe-stringcchvprintfexa ;
...
CErr_Details& CErr_Details::operator << (const dword    _err_code) {
	...
	if (ERROR_RESOURCE_LANG_NOT_FOUND == ::GetLastError()) {
		...
		m_desc.Format(_T("No desc for the error code: %d"), _err_code);// _T("#undef;");
	}
...
CErr_Details& CErr_Details::operator << (const err_code  _err_code) {
	...
	if (b_result) {
		...
		m_desc.Format(_T("No desc for the error code: 0x%x"), _err_code);// _T("#undef;");
	}
	return *this;
}
