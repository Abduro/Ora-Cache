...
CString CPercent_F::Print (const e_print_f _e_opt_f, const e_print) const {
	...
	CString cs_out;
	if (e_print_f::e_as_is  == _e_opt_f) cs_out.Format(pc_sz_pat_ns, (_pc_sz)__CLASS__, TString().Float(this->Get()));
	if (e_print_f::e_as_int == _e_opt_f) cs_out.Format(pc_sz_pat_ns, (_pc_sz)__CLASS__, TString().Long(this->GetAsInt()));
