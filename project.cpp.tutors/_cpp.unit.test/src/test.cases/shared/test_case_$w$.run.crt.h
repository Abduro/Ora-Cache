...
/*
	...
	This is Ebo Pack OpenGL tutorials' c-runtime (CRT) thread runner wrapper interface declaration file for using in test cases adapters;
*/
...
namespace ebo { namespace boo { namespace test { namespace threads {
	...
	class CTstCrtRunner {
	public:
		 CTstCrtRunner (void); CTstCrtRunner (const CTstCrtRunner&) = delete; CTstCrtRunner (CTstCrtRunner&&) = delete;
		~CTstCrtRunner (void); // if c-runtime worker thread is still running it should be automatically stopped in its destructor;
	...
	private:
		CTstCrtRunner& operator = (const CTstCrtRunner&) = delete; CTstCrtRunner& operator = (CTstCrtRunner&&) = delete;
		CCrtRunner   m_runner;
