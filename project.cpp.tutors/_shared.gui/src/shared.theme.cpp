...
CBkgnd::CBkgnd (void)  : m_bkgnd(4, 0.0f), m_is_read(false) {}

const rgb_color  CBkgnd::Rgba (void) const { return (rgb_color)CHex((_pc_sz)Get_registry().Value(e_element::e_bkgnd)); }
const s_flt_clr& CBkgnd::Float (void) const {

	if (this->m_is_read)
		return this->m_bkgnd;

	const rgb_color clr = this->Rgba();
...
const CBkgnd& CTheme::Bkgnd (void) const {
	return this->m_bkgnd;
}
