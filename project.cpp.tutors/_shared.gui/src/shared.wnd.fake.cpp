...
CMsgWnd:: CMsgWnd (const bool _b_verbose) : TWindow(), m_h_dc(0), m_verbose(_b_verbose) { this->m_error >> __CLASS__ << __METHOD__ << __e_not_inited;

	TWindow::Create(HWND_MESSAGE);

	if (false == TWindow::IsWindow()) {
		this->m_error.Last();
		if (this->m_verbose) __trace_err_3(_T("%s\n"), (_pc_sz) this->Error().Print(TError::e_req)); return;
	}
	else {
		this->m_error << __s_ok;
		this->m_h_dc = TWindow::GetDC();
	//	__empty_ln();
		if (this->m_verbose) __trace_info_3(_T("context device: handle=%s;\n"),  TString()._addr_of(this->Get_ctx()));
	}
}
